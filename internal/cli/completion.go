    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        for _, sh := range shells {
            ...
            case "bash":
                ...
                if err := writeBashCompletion(p); err != nil { return err }
                fmt.Fprintln(cmd.OutOrStdout(), bashInstruction(p))
            case "zsh":
                p := filepath.Join(target, "tarragon.zsh")
                if err := writeZshCompletion(p); err != nil { return err }
                fmt.Fprintln(cmd.OutOrStdout(), zshInstruction(p))
            case "fish":
                p := filepath.Join(target, "tarragon.fish")
                if err := writeFishCompletion(p); err != nil { return err }
                fmt.Fprintln(cmd.OutOrStdout(), fishInstruction(p))
            case "powershell", "pwsh", "ps":
                p := filepath.Join(target, "tarragon.ps1")
                if err := writePSCompletion(p); err != nil { return err }
                fmt.Fprintln(cmd.OutOrStdout(), psInstruction(p))
            default:
