...
func TestDiscoverLoadsPluginConfig(t *testing.T) {
    ...
    if err := os.MkdirAll(plugDir, 0o755); err != nil { t.Fatal(err) }
    toml := []byte("" +
        "name=\"example\"\n" +
        "description=\"demo\"\n" +
        "enabled=true\n" +
    ...
    if err := os.WriteFile(filepath.Join(plugDir, "plugin.toml"), toml, 0o644); err != nil { t.Fatal(err) }

    m := NewManager(root)
    if err := m.Discover(); err != nil { t.Fatalf("discover: %v", err) }
    p, ok := m.Plugins["example"]
    if !ok { t.Fatalf("plugin not found in manager") }
...
func TestDefaultDirContainsHome(t *testing.T) {
    got := DefaultDir()
    if got == "" { t.Fatalf("DefaultDir is empty") }
